...
const { initializeFunctionsAgent } = require('./agents');
...
class WorkflowClient extends OpenAIClient {
  ...
  async initialize({ user, sender, message, onAgentAction, onChainEnd, signal }) {
    ...
    // initialize agent
    const initializer = initializeFunctionsAgent;
    ...
    logger.debug('[WorkflowClient] Loaded agent.');
  }
  ...
  async executorCall(message, { signal, stream, onToolStart, onToolEnd }) {
    ...
    for (let attempts = 1; attempts <= maxAttempts; attempts++) {
      ...
      try {
        ...
        logger.error('[WorkflowClient] executorCall error:', err);
  ...
  async sendMessage(message, opts = {}) {
    ...
    const completionMode = opts.isEdited;
    if (completionMode) {
